...
if(!empty($_POST))
{
	...
	if($_FILES['fileToUpload']['name'] != "")
	{
		...
		$newname = dirname('_FILE_').'\\user-data\\img\\'.md5($loggedInUser->email).'.'.end($temp);
		if($uploadOk)
		{
			if(move_uploaded_file($_FILES['fileToUpload']['tmp_name'], $newname))
			{
				$pic = md5($loggedInUser->email).'.'.end($temp);
	...
	if(!empty($_POST["passwordc"]) and !empty($_POST["passwordcheck"]))
	{
...
echo "
<div id='regbox'>
<form name='updateAccount' action='".$_SERVER['PHP_SELF']."' method='post' enctype='multipart/form-data'>
<p>
<br>
<label>Enter your Password to make any changes:</label>
...
<p><br>
<label>Select image to upload as your profile picture:</label>
